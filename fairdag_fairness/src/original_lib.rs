    ...
    fn readd_nodes_to_graph(&mut self, to_readd: Vec<TxDigest>, target_graph_idx: usize) {
        ...
        for d in &to_readd {
            ...
            if ap >= self.solid_threshold {
                ...
                // Still below threshold — per protocol, do NOT add to graph.
                // Keep as Blank and defer to pending_readd for a future graph.
                self.nodes.get_mut(&d).unwrap().node_type = NodeType::Blank;
                self.nodes.get_mut(&d).unwrap().graph_index = None;
                self.pending_readd.push(d);
                continue;
            }
            ...
            for d2 in existing_nodes {
                ...
                // Mark all replicas that contributed as counted so that
                // future incremental weight updates do not double-count.
                let pair = (d.min(d2), d.max(d2));
                let node1 = self.nodes.get(&d).unwrap();
                let node2 = self.nodes.get(&d2).unwrap();
                let replica_set = self.graphs[target_graph_idx]
                    .counted_replicas
                    .entry(pair)
                    .or_default();
                for (&i, _) in &node1.committed_ois {
