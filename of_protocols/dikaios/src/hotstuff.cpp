...
void HotStuffBase::process_local_order(const LocalOrder &local_order){
    if(on_receive_local_order(local_order, pmaker->get_parents())==true){
        ...
        pmaker->beat().then([this, ordered_txs = std::move(ordered_txs)](ReplicaID proposer) {
                if (proposer == get_id()) {
                    on_propose(ordered_txs, pmaker->get_parents());
                }
            });
    }
}
