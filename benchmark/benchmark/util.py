...
def execute_command_quietly(command_line_list):
    ...
    with open(os.devnull, "wb") as devnull:
        try:
            ...
            if command_line_list[0] == "preserve" and "-c" in command_line_list:
                print(f"Warning: {e.cmd} failed with exit code {e.returncode}, "
                      f"reservation probably already finished.")
                return
