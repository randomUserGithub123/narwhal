...
class HotStuffCore {
    ...
    // 用于缓存两个交易之间的相对顺序结果
    struct TxPairHash {
        size_t operator()(const std::pair<uint256_t, uint256_t>& p) const {
            return std::hash<uint256_t>()(p.first) ^ (std::hash<uint256_t>()(p.second) << 1);
        }
    };
    ...
    struct TxPosition {
        size_t index;
        bool exists;
        TxPosition(): index(0), exists(false) {}
        TxPosition(size_t i): index(i), exists(true) {}
    };

    using OrderCache = std::unordered_map<std::pair<uint256_t, uint256_t>, bool, TxPairHash>;
    using TxPositionMap = std::unordered_map<uint256_t, std::unordered_map<ReplicaID, TxPosition>>;

    std::unordered_map<uint256_t, size_t> find_fixed_positions(
        const std::unordered_map<ReplicaID, std::vector<uint256_t>>& orders,
        const TxPositionMap& position_cache);
        
    bool analyze_relative_order(
        const uint256_t& tx1, 
        const uint256_t& tx2,
        const std::unordered_map<ReplicaID, std::vector<uint256_t>>& orders,
        const std::unordered_map<ReplicaID, double>& credits,
        OrderCache& order_cache,
        const TxPositionMap& position_cache);

    std::vector<std::vector<uint256_t>> group_transactions(
        const std::unordered_map<uint256_t, size_t>& fixed_positions,
        const std::unordered_set<uint256_t>& all_txs,
        const std::unordered_map<ReplicaID, std::vector<uint256_t>>& orders,
        const std::unordered_map<ReplicaID, double>& credits,
        OrderCache& order_cache,                   // 添加缓存参数
        const TxPositionMap& position_cache);      // 添加位置缓存参数

    std::vector<uint256_t> order_group(
        const std::vector<uint256_t>& group,
        const std::unordered_map<ReplicaID, std::vector<uint256_t>>& orders,
        const std::unordered_map<ReplicaID, double>& credits,
        OrderCache& order_cache,                   // 添加缓存参数
        const TxPositionMap& position_cache);      // 添加位置缓存参数

    std::vector<uint256_t> fair_propose();
    // myOrder <
